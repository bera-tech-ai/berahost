...
import { db, paymentTransactionsTable, usersTable, platformSettingsTable } from "@workspace/db";
...
async function getPayheroCredentials(): Promise<{ authToken: string; channelId: string } | null> {
  const authToken = process.env.PAYHERO_AUTH_TOKEN;
  const channelId = process.env.PAYHERO_CHANNEL_ID;
  if (authToken && channelId) return { authToken, channelId };

  const rows = await db.select().from(platformSettingsTable).where(
    sql`${platformSettingsTable.key} IN ('payhero_auth_token', 'payhero_channel_id')`
  );
  const map = Object.fromEntries(rows.map(r => [r.key, String(r.value ?? "")]));
  const dbToken = map["payhero_auth_token"];
  const dbChannel = map["payhero_channel_id"];
  if (dbToken && dbChannel) return { authToken: dbToken, channelId: dbChannel };
  return null;
}
...
router.post("/payments/initiate", requireAny("payments"), async (req, res): Promise<void> => {
  ...
  const creds = await getPayheroCredentials();
  if (!creds) {
    res.status(500).json({ error: "Payment system not configured. Ask your admin to add PayHero credentials in Platform Settings." });
    return;
  }
  const { authToken, channelId } = creds;
