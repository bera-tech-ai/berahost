...
const adminSubLinks = [
  ...
  { name: "Deployments",       href: "/admin/deployments", icon: Activity },
  { name: "Transactions",      href: "/admin/payments",  icon: TrendingUp },
  { name: "Vouchers",          href: "/admin/vouchers",  icon: Ticket },
  { name: "Support Tickets",   href: "/admin/support",   icon: LifeBuoy },
  { name: "PayHero Settings",  href: "/admin/settings",  icon: PayIcon },
];
...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  const isNavActive = (path: string) => {
    ...
    if (path !== "/dashboard" && path !== "/admin" && location.startsWith(path)) return true;
    return false;
  };
      ...
      {user?.isAdmin && (
        <>
          ...
          <button
            ...
            className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-mono transition-all border w-full text-left ${
          ...
          {adminExpanded && (
            <div className="ml-3 flex flex-col gap-0.5 border-l border-destructive/20 pl-3">
              {adminSubLinks.map((item) => (
                <Link key={item.href} href={item.href} onClick={onClick}>
                  <div className={`flex items-center gap-2 px-3 py-2 rounded-md text-xs font-mono transition-all ${
                    ...
                    {item.name}
                    {isNavActive(item.href) && <ChevronRight className="ml-auto h-3 w-3 text-destructive/50" />}
                  </div>
                </Link>
              ))}
            </div>
